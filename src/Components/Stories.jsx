         ...
         <Splide options={splideOptions}>
            {news.map((val, i) =>(
               <SplideSlide key={i} className='mb-0.5'>
                 <div className='relative grid items-center gap-4 pb-2
                 rounded-lg shadow shadow-slate-200 ring-1 ring-slate-200'>

                    <div className='flex items-center justify-center'><img src={val.img} alt={`img/story/${i}`} 
                    className='w-full h-auto object-cover shadow-md shadow-slate-200 rounded-tl-tlg
                    rounded-tr-lg'/></div>
                    ...
                    <div className='flex items-center justify-between w-full px-4'>
                        ...
                        <div className='flex items-center gap-0.5'><HashtagIcon className='icon-style text-blue-600'/><span className='text-xs text-blue-600'>{val.by}</span></div>
